...
- **AC3** — `redactDeep` replaces values of keys matching `(?<![a-z0-9])(password|passwd|secret|token|api[_-]?key|apikey|auth[_-]?token|credential)(?![a-z0-9])` case-insensitively with `"[REDACTED]"`, leaves `keyboard`/`monkey` untouched, and catches underscore-attached names (`db_password`, `access_token`, `SECRET_KEY`) that the original `\b` regex missed (boundary fixture, §5.2, debt D3).
