 ...
 *
 * Normalized events: {task-updated, agent-status-changed, run-updated,
 * budget:breach}, deduped by type+id against updatedAt/seq so the gateway's
 * heavy task re-upserts do not reach browsers. Envelope-seq gaps force a
 * `resync` hint broadcast. The budget-breach type is slice 3 of the budget
 * ledger (docs/briefs/budget-ledger.md §3.7): the dispatcher emits latched
 * `budget:breach` frames directly via routes/sse-routes; the bridge additionally
 * normalizes an additive `budget.breach` gateway envelope into the SAME frame
 * shape (lib/budget-enforcement.js buildBudgetBreachFrame) so a future relay
 * produces identical wire frames. Purely additive — existing consumers see no
 * change.
 */
...
const { buildBudgetBreachFrame } = require('./budget-enforcement');
 ...
 *  'budget.breach' is the additive budget-ledger slice-3 envelope (never sent by
 *  today's gateway; normalized when present so relayed breaches fan out). */
const CONSUMED_EVENTS = new Set(['task', 'agent', 'session.tool', 'budget.breach']);
...
function normalizeGatewayEvent(eventName, payload) {
  ...
  if (eventName === 'budget.breach') {
    // Additive budget-ledger slice-3 envelope: payload carries the same field
    // set as the dispatcher's direct frame (see buildBudgetBreachFrame), so
    // both producers emit byte-compatible `budget:breach` frames.
    const frame = buildBudgetBreachFrame({
      event: { event_kind: payload.event_kind, created_at: payload.timestamp },
      // Envelope uses wire field names (budget_id/budget_name); the frame
      // builder reads the row shape (id/name) — map explicitly so identity
      // checks pass and the title-bearing budget_name survives.
      budget: { ...payload, id: payload.budget_id, name: payload.budget_name },
      key: payload.period_key,
      spendUsd: payload.spend_usd,
      spendTokens: payload.spend_tokens,
    });
    if (!frame) return null;
    return {
      type: frame.type,
      id: frame.id,
      updatedAt: Number(payload.seq || 0) || Date.parse(frame.timestamp) || 0,
      data: frame,
    };
  }
