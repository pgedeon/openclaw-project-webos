 ...
 *
 * Deny-regex is pinned by the brief (§5.2 / AC3 fixture):
 *   \b(password|passwd|secret|token|api[_-]?key|apikey|auth[_-]?token|credential)\b
 * case-insensitive, word boundaries so keyboard/monkey never trip.
 * Known limitation (accepted, spec-as-written): \b does not fire after an
 * underscore or letter because both are word characters, so snake_case keys
 * like postgres_password or access_token do NOT match this regex. That is why
 * layer 1 exists: the five password-type settings keys are excluded
 * structurally by redactSettings(), not by this regex. The deny pass is the
 * second net, not the first.
 */

// §5.2 pinned deny-regex — word-boundary, case-insensitive. AC3 pins
// keyboard/monkey as must-NOT-trip fixtures; do not widen without re-running
// those fixtures.
const DENY_RE = /\b(?:password|passwd|secret|token|api[_-]?key|apikey|auth[_-]?token|credential)\b/i;
