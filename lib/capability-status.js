...
const FEATURE_REASONS = {
  ...
  // snapshots: the snapshot/restore slice answers 503-flavored degrades
  // (deliberately stricter than budgets' HTTP-200 contract — brief §4.1);
  // the reason vocabulary is the same house pair the routes have always
  // pinned (tests/test-snapshot-routes.js AC7): pool absent ⇒ 'no_database',
  // thrown query ⇒ 'query_failed'. HTTP status passthrough stays the
  // route's job — the resolver only names the body.
  snapshots: {
    unreachable: 'query_failed',
    misconfigured: 'no_database',
  },
...
const FEATURE_UI_CLAUSES = {
  ...
  // fleet: the panel's down-state means every fleet input (health + CLI
  // agents) failed its fetch — the honest clause names the gateway, not a
  // database the view cannot observe from a failed fetch.
  fleet: {
    unreachable: 'gateway not reachable',
  },
  // cost: the panel's down-state fires on a failed fetch OR an
  // {available:false} body — both mean the cost series could not be read;
  // the honest clause names the data, not a specific backend.
  cost: {
    unreachable: 'cost data unreachable',
  },
};
