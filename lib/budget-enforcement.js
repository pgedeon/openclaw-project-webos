...
// Reverse map for surfacing: which enforced action a recorded event kind
// corresponds to. Used by buildBudgetBreachFrame so an SSE frame's `action`
// always reflects what the system DID (the most-restrictive enforced action),
// even when the individual budget's own action_on_exceed was softer.
const ACTION_BY_EVENT_KIND = Object.freeze({
  warned: 'warn',
  paused: 'pause_new_runs',
...
/**
 * Build one `budget:breach` SSE frame from a latched budget_events row plus
 * the evaluation snapshot that produced it. Pure; shared by the dispatcher's
 * direct fan-out (gateway-workflow-dispatcher-v2.js) and the gateway bridge's
 * additive `budget.breach` envelope normalizer (lib/gateway-bridge.js) so both
 * producers emit byte-compatible frames on /api/events/stream.
 *
 * @param {{event:{event_kind:string, created_at?:string|null},
 *         budget:object, key:string, spendUsd?:number, spendTokens?:number}} item
 * @returns {object|null} frame, or null when the row/budget identity is missing
 */
function buildBudgetBreachFrame({ event, budget, key, spendUsd = 0, spendTokens = 0 }) {
  if (!event || !event.event_kind || !budget || !budget.id || !key) return null;
  const action = ACTION_BY_EVENT_KIND[event.event_kind] || budget.action_on_exceed || null;
  ...
  const usd = Math.round(Number(spendUsd || 0) * 1e6) / 1e6;
  const tokens = Number(spendTokens || 0);
  const capLabel = capUsd != null
    ? `$${capUsd.toFixed(2)}`
    : capTokens != null ? `${capTokens.toLocaleString('en-US')} tokens` : 'unlimited';
  const spendLabel = capUsd != null ? `$${usd.toFixed(2)}` : `${tokens.toLocaleString('en-US')} tokens`;
  return {
    type: 'budget:breach',
    id: `${budget.id}:${key}:${event.event_kind}`,
    budget_id: budget.id,
    budget_name: budget.name,
    scope: budget.scope,
    scope_id: budget.scope_id == null ? null : String(budget.scope_id),
    ...
    message: `${action || event.event_kind} enforced at ${spendLabel} of ${capLabel} cap (${key})`,
...
function createBudgetEnforcement(pool, options = {}) {
  ...
  /**
   * Record breach events for every breached budget entry, idempotently, and
   * return the rows that ACTUALLY inserted (latched duplicates excluded) so
   * callers can throttle SSE fan-out to exactly one frame per
   * (budget_id, period_key, event_kind) — the UNIQUE latch is the throttle.
   * Each item carries the evaluation snapshot needed for frame building.
   ...
   * @returns {Promise<Array<{event, budget, key, spendUsd, spendTokens}>>}
   */
  async function collectBreachEventRows(breached, action, extraDetail = {}) {
  ...
  /**
   * Count-record variant of collectBreachEventRows (slice-2 shape preserved:
   * returns the number of rows actually inserted, 0 when everything latched).
   */
  async function recordBreachEvents(breached, action, extraDetail = {}) {
    return (await collectBreachEventRows(breached, action, extraDetail)).length;
  }
  ...
  async function hardStopInFlight(breached) {
    ...
    for (const entry of breached || []) {
      ...
      try {
        ...
        await ensureRecoveredMarker(entry.budget, entry.key);
        const eventRow = await insertEvent(entry.budget.id, entry.key, 'hard_stopped', {
          action: 'hard_stop',
          spend_usd: Math.round(entry.spendUsd * 1e6) / 1e6,
          spend_tokens: entry.spendTokens,
          cap_usd: entry.budget.cap_usd ?? null,
        ...
        // eventRow feeds slice-3 SSE surfacing: the hard_stopped insert here
        // is the PRIMARY latch for hard_stop breaches (recordBreachEvents'
        // later attempt for the same key conflicts away), so the frame must
        // be built from THIS row or hard-stop breaches would never surface.
        results.push({
...
module.exports = {
  DEFAULT_TTL_MS,
  EVENT_KINDS,
  ACTION_BY_EVENT_KIND,
  createBudgetEnforcement,
  normalizeBudgetRow,
  scopePredicate,
  buildBudgetBreachFrame,
};
