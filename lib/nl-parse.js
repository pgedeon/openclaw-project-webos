 ...
 *   - Flagship "spawn agent for X" refuses with task_create_unavailable:
 *     task creation is not a v1 catalog kind (§5 flagship scoping), so the
 *     honest answer is a named refusal, never a guessed envelope.
...
(function (root, factory) {
   ...
   *   - refusal: {unmatched:true, reason:'batch_not_supported'|'temporal_not_supported'|'task_create_unavailable'}
  ...
  function parseIntent(utterance, context) {
    ...
    // 2b. Flagship honesty (§5 "The flagship utterance, honestly scoped"):
    //     "spawn agent for X, report when done" decomposes into task.create
    //     + run.dispatch, and task.create is NOT a v1 catalog kind. Refuse
    //     with a named reason instead of guessing an envelope — the palette
    //     renders this as a dedicated card with a deep link to Tasks view.
    if (/^(spawn|create|new)\b/i.test(lower) && /\b(agent|task)\b/.test(lower)) {
      return unmatched('task_create_unavailable');
    }
